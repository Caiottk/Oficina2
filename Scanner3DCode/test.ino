...
void loop() 
{
  ...
  int noZteps=20; //No of z-steps per rotation.  Distance = noSteps*0.05mm/step
  // int zCounts=(200/1*vertDistance)/noZteps; //Total number of zCounts until z-axis returns home
  int zCounts=noZteps;
  //int thetaCounts=400;
  int thetaCounts=200;
  ...
  for (int j=0; j<vertDistance; j++) //Rotate z axis loop
  {
    ...
    Serial.println();
    rotateMotor(PIN_STEP_MZ, noZteps); //Move z carriage up one step
    delay(1000);
  ...
  {
    rotateMotor(PIN_STEP_MZ, noZteps);
    delay(10);
  }
...
int getVerticalDistance() {
  ...
  while (readAnalogSensor() < THRESHOLD) { // THRESHOLD é o valor que determina se o objeto foi detectado
