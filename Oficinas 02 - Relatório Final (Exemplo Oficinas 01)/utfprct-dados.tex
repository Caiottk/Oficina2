...
%% Data da defesa
\Dia{20}%% Dia (opcional: usado na ficha catalográfica apenas)
\MesPorExtenso{dezembro}%% mês por extenso (opcional: usado na ficha catalográfica apenas)
\Ano{2024}%% Ano

%% Palavras-chave e keywords (máximo 5)
\NumeroDePalavrasChave{5}%% Número de palavras-chave 
\PalavraChaveA{Impressão 3D}%% Palavra-chave A
\PalavraChaveB{Scanner}%% Palavra-chave B
\PalavraChaveC{Gestão de Projeto}%% Palavra-chave C
\PalavraChaveD{Digitalização}%% Palavra-chave D
\PalavraChaveE{}%% Palavra-chave E

\NumeroDeKeywords{\imprimirnumerodepalavraschave}%% Número de keywords (mesmo que palavras-chave)
\KeywordA{3D Printer}%% Keyword A
\KeywordB{Scanner}%% Keyword B
\KeywordC{Project Management}%% Keyword C
\KeywordD{}%% Keyword D
\KeywordE{}%% Keyword E
...
\NomeDoAutor{Caio Andrade}%% Nome completo do(a) autor(a)
\SobrenomeDoAutor{Andrade}%% Último nome do(a) autor(a)
\PrenomeDoAutor{Caio Andrade}%% Nome do(a) autor(a) sem último nome
...
\NomeDoAutorDois{Caio Miritz}%% Nome completo do(a) autor(a) 2
\SobrenomeDoAutorDois{Miritz}%% Último nome do(a) autor(a) 2
\PrenomeDoAutorDois{Caio Miritz}%% Nome do(a) autor(a) 2 sem último nome

%%% Autor(a) 3 (opcional)
%% *Considera apenas se "\TipoDeDocumento" == "Trabalho de Conclusão de Curso"
\AtribuiAutorTres{true}%% Insere ou remove autor(a) 3: "true" ou "false"
\NomeDoAutorTres{Rogerio Slabiski Melo}%% Nome completo do(a) autor(a) 3
\SobrenomeDoAutorTres{Melo}%% Último nome do(a) autor(a) 3
\PrenomeDoAutorTres{Rogerio Slabiski}%% Nome do(a) autor(a) 3 sem último nome
...
\NomeDoOrientador{Cesar Manuel Vargas Benitez}%% Nome completo do(a) orientador(a)
\SobrenomeDoOrienador{Benitez}%% Último nome do(a) orientador(a)
\PrenomeDoOrientador{Cesar Manuel}%% Nome do(a) orientador(a) sem último nome
...
% \TituloDoCoorientador{Prof.}%% Título do(a) coorientador(a)
% \NomeDoCoorientador{Ronnier Frates Rohrich}%% Nome completo do(a) coorientador(a)
% \SobrenomeDoCoorienador{Rohrich}%% Último nome do(a) coorientador(a)
% \PrenomeDoCoorientador{Ronnier Frates}%% Nome do(a) coorientador(a) sem último nome
...
\DescricaoDoDocumento{ Relatório Final da disciplina Oficina de Integração 2, do curso de \imprimircurso, apresentado ao professor que ministra a mesma na Universidade Tecnológica Federal do Paraná como requisito parcial para obtenção da aprovação na disciplina.
}

%Default: \DescricaoDoDocumento{
